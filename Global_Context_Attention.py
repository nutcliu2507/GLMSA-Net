from tensorflow.keras.layers import (Conv1D, Conv2D, Conv3D, Reshape, Activation, Softmax, Permute, add,
                                     dot, LayerNormalization)
import tensorflow.keras.backend as K
from group_norm import GroupNormalization


def global_context_block_GL(ip, reduction_ratio=4, transform_activation='linear'):
...
def global_context_block_LN(ip, reduction_ratio=4, transform_activation='linear'):
    """
...
def _convND(ip, rank, channels, kernel=1):
    ...
    if rank == 3:
        ...
        x = Conv3D(channels, (kernel, kernel, kernel), padding='same', use_bias=False, kernel_initializer='he_normal')(
            ip)
