...
from Global_Context_Attention import global_context_block_GL, global_context_block_LN
...
def residual_conv_block(filters, stage, block, strides=(1, 1), attention=True, cut='pre', reduce_ratio=16):
    ...
    def layer(input_tensor):
        ...
        #     #            x = attention(x)
        #     se_input = x
        #     x = layers.GlobalAveragePooling2D(name=conv_name + 'GAP')(x)
        #     x = layers.Reshape((1, 1, filters))(x)
        #     x = layers.Dense(filters // reduce_ratio, name=conv_name + 'FC1', activation='relu')(x)  # FC1
        #     x = layers.Dense(filters, activation='sigmoid', name=conv_name + 'FC2')(x)  # FC2
        #     x = layers.Multiply(name=conv_name + 'M')([x, se_input])  # Scale

        # x = global_context_block_LN(x, reduction_ratio=4)
...
def residual_bottleneck_block(filters, stage, block, strides=None, attention=None, cut='pre', reduce_ratio=8):
    ...
    def layer(input_tensor):
        ...
        #     se_input = x
        #     x = layers.GlobalAveragePooling2D(name=conv_name + 'GAP')(x)
        #     x = layers.Reshape((1, 1, filters * 4))(x)
        #     x = layers.Dense(filters * 4 // reduce_ratio, name=conv_name + 'FC1', activation='relu')(x)  # FC1
        #     x = layers.Dense(filters * 4, activation='sigmoid', name=conv_name + 'FC2')(x)  # FC2
        #     x = layers.Multiply(name=conv_name + 'M')([x, se_input])  # Scale
        # x = global_context_block_LN(x, reduction_ratio=4)
...
def ResNet(model_params, input_shape=None, input_tensor=None, include_top=True,
    ...
    # x = layers.Conv2D(init_filters, (3, 3), strides=(2, 2), padding='valid', name='conv2dds')(x)
    # resnet body
